    ...
    companion object {
        fun detect(text: String, options: FormattingOptions?): Formatting {
            ...
            if (options != null) {
                ...
                if (var10000 != null) {
                    lineWidth = var10000.toInt()
                } else {
                    val lineWidthFraction: Int? = max(lines as Iterable<*>?. asSequence < T ? > ().map(null.INSTANCE as Function1<*, *>?))
                    lineWidth = Math.max(80, Math.round((lineWidthFraction
                            ?: 0).toFloat() / 10.0f) * 10)
                }
            }
